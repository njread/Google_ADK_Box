...
Box_hub_agent = LlmAgent(
    ...
    Sub Agent Guidance:
    1. For questions related to credit policy, loan compliance, policy exceptions, or home loan 
    ...
    2. If an agent returns an error message (e.g., starting with 'API Error:' or 'An unexpected 
    ...
    3. You do not always need to respond. If there is another agent that has provided a response, 
       you can wait until you are explicitly called by the user.
    """,
    sub_agents=[Box_hub_Credit_Policy_agent]
)